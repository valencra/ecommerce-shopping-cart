...
import static com.acme.ecommerce.controller.WebConstants.COST_PER_ITEM;
import static com.acme.ecommerce.controller.WebConstants.LIST_COUNTRIES;
import static com.acme.ecommerce.controller.WebConstants.LIST_MONTHS;
import static com.acme.ecommerce.controller.WebConstants.LIST_STATES;
import static com.acme.ecommerce.controller.WebConstants.LIST_YEARS;

import com.acme.ecommerce.domain.Address;
import com.acme.ecommerce.domain.CombinedBilling;
...
public class CheckoutController {
  ...
  String postCouponCode(
      Model model, @Valid @ModelAttribute(value = "couponCode") CouponCode couponCode,
  ...
  String postShipping(@ModelAttribute(value = "shippingAddress") @Valid Address shippingAddress,
                      final BindingResult result, RedirectAttributes redirectAttributes) {

    if (result.hasErrors()) {
      logger.error("Errors on fields: " + result.getFieldErrorCount());
      redirectAttributes
          .addFlashAttribute("org.springframework.validation.BindingResult.shippingAddress",
              result);
  ...
  String postBilling(
      @ModelAttribute(value = "billingObject") @Valid CombinedBilling combinedBilling,
      final BindingResult result, RedirectAttributes redirectAttributes) {

    if (result.hasErrors()) {
      redirectAttributes
          .addFlashAttribute("org.springframework.validation.BindingResult.billingObject", result);
      ...
      if (purchase != null) {
        ...
        purchase
            .setOrderNumber(String.valueOf(ThreadLocalRandom.current().nextInt(1000000, 9999999)));
        sCart.setPurchase(purchaseService.save(purchase));
      } else {
        logger.error("No purchases Found!");
        return ("redirect:/error");
  ...
  public void getFile(HttpServletResponse response) {
    ...
    try {
      ...
      if (purchase != null) {
        ...
        InputStream
            inputStream =
            new BufferedInputStream(new ByteArrayInputStream(htmlContent.getBytes()));
      ...
      logger
          .info("Error writing file to output stream. Filename was '{}'", "email_receipt.html", ex);
  ...
  private BigDecimal computeShippingCost(Purchase purchase) {
    ...
    for (ProductPurchase pp : purchase.getProductPurchases()) {
      BigDecimal
          itemShippingCost =
          new BigDecimal(0).add(COST_PER_ITEM.multiply(new BigDecimal(pp.getQuantity())));
      logger.debug(
          "cart has " + pp.getQuantity() + " of " + pp.getProduct().getName() + " shipping cost of "
              + itemShippingCost);
